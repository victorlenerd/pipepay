    ...
    var controller = {
        findByParam: function (req, res, next, id) {
            model.findById(id, function (err, doc) {
                if (err) res.status(404).send({ status: false, error: { message: "Not found" } });
                req.docId = doc.id;
                next();
            });
        },
        createOne: function (req, res) {
            ...
            model.create(body, function (err, doc) {
                if (err) res.status(401).send({ status: false, error: { ...err } });
                res.status(200).send({ status: true, data: doc });
            });
        },
        ...
        getAll: function (req, res) {
            model.find({}, function (err, docs) {
                if (err) res.status(401).send({ status: false, error: { ...err } });;
                res.status(200).send({ status: true, data: docs });
            });
        },
        getOne: function (req, res) {
            ...
            model.find({ _id: id }, function (err, docs) {
                if (err) res.status(401).send({ status: false, error: { ...err } });;
                res.status(200).send({ status: true, data: docs[0] });
            });
        },
